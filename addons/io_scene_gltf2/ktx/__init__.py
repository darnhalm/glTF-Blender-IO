...
Pinned KTX tools are bundled and verified locally; no download is performed.
"""
...
class KTX2_OT_install_tools(bpy.types.Operator):
    """Verify bundled KTX tools for KTX2 texture support"""
    bl_idname = "heritage3d_ktx2.install_tools"
    bl_label = "Check Bundled KTX Tools"
    bl_description = "Verify and prepare bundled KTX-Software tools for KTX2 encoding/decoding"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        try:
            self.report({'INFO'}, "Checking bundled KTX tools...")
            ...
            if success:
                ...
                self.report({'INFO'}, "Bundled KTX tools verified!")
            else:
...
class KTX2_OT_check_installation(bpy.types.Operator):
    ...
    bl_idname = "heritage3d_ktx2.check_installation"
    ...
    def execute(self, context):
        ...
        if check_tools_available():
            ...
            self.report({'WARNING'}, "KTX tools are not available. Click 'Check Bundled KTX Tools' to verify the package.")
        return {'FINISHED'}
...
class KTX2ExportProperties(bpy.types.PropertyGroup):
    ...
    envmap_mode: bpy.props.EnumProperty(name="Environment compression",
        items=[('ETC1S', 'ETC1S', ''), ('UASTC', 'UASTC', '')], default='ETC1S')
    envmap_quality: bpy.props.IntProperty(name="Environment quality", min=1, max=255, default=128)
    envmap_uastc_quality: bpy.props.IntProperty(name="Environment quality", min=0, max=4, default=2)
...
def draw_install_tools_ui(layout):
    ...
    if _installation_in_progress:
        box.label(text="Verifying bundled tools...")
        box.enabled = False
    else:
        col = box.column(align=True)
        col.operator("heritage3d_ktx2.install_tools", icon='IMPORT')
        col.operator("heritage3d_ktx2.check_installation", icon='FILE_REFRESH')
        col.label(text="Tools are bundled in the platform package", icon='URL')
    ...
    if body is not None:
        if not check_tools_available():
            ...
            if props.export_environment_map:
                ...
                body.prop(props, 'envmap_mode')
                body.prop(props, 'envmap_quality' if props.envmap_mode == 'ETC1S' else 'envmap_uastc_quality')
...
class glTF2ExportUserExtension:
    ...
    def post_export_hook(self, export_settings):
        # Unlike post_export_callbacks, this hook runs after the file is written.
    ...
    def gather_gltf_extensions_hook(self, gltf, export_settings):
        ...
        # Finalized synchronously by post_export_hook after file writing.
...
def glTF2_post_export_callback(export_settings):
    ...
    pass  # File finalization is handled by post_export_hook, after writing.
...
def _run_post_export(export_settings):
    ...
    if export_settings['gltf_format'] == 'GLB':
        ...
        _post_process_gltf_envmap(filepath, export_settings['gltf_format'])
