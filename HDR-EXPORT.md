# HERITAGE3D HDR fork — Blender 5.2 preview
...
and choose resolution and encoding quality. HDR defaults to off; with it off,
the original upstream exporter is used directly.
...
graphs, UDIM, multilayer EXR and environment maps are outside this preview.
...
folder in Blender's `scripts/addons_core`. **Overlay the archive's io_scene_gltf2
...
The separate HERITAGE3D HDR add-on is not required.

The initial binary package targets macOS Apple Silicon. It includes pinned
KTX-Software 5.0.0-rc2 and libktx, checks their SHA-256 and runs a verified copy
...
Build:
```
python3 tools/build_hdr_preview.py --ktx-prefix /path/to/ktx-install-prefix
```
The prefix must contain bin/ktx and lib/libktx.5.dylib. No personal paths or
